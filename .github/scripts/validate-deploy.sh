...
#NAMESPACE=$(jq -r '.namespace // "cpd-operators"' gitops-output.json)
...
count=0
until kubectl get subscription "ibm-db2u-operator" -n ${NAMESPACE} || [[ $count -eq 20 ]]; do
  echo "Waiting for subscription/ibm-db2u-operator in ${NAMESPACE}"
  count=$((count + 1))
  sleep 15
done

if [[ $count -eq 20 ]]; then
  echo "Timed out waiting for subscription/ibm-db2u-operator in ${NAMESPACE}"
